...
begin
  ...
  try
    ...
    begin
      ...
      begin
        TSimpleRTTI<T>.New(aInstance).GetDateType(Key, lTipo);
        case lTipo of
          ftUnknown,
          ...
          ftBlob:
          begin
            lStream := TSimpleCommon.New.OleVariantToMemoryStream(DictionaryFields.Items[key]);
            try
...
function TSimpleDAO<T>.Find(var obj: T; Key: String; Value: Variant)
  : iSimpleDAO<T>;
var
  lSQL: String;
begin
  Result := Self;
  obj := T.Create;
  TSimpleSQL<T>.New(nil).Where(Key + ' = :' + Key).Select(lSQL);
  FQuery.SQL.Clear;
  FQuery.SQL.Add(lSQL);
  FQuery.Params.ParamByName(Key).Value := Value;
  FQuery.Open;
  TSimpleRTTI<T>.New(nil).DataSetToEntity(FQuery.DataSet, obj);
end;
