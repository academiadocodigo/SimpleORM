...
{$ENDIF}
    ...
    function GetDataType(const aCampo: String; var aType: TFieldType)
      : iSimpleRTTI<T>;
  end;
...
{ TODO -oAlessandro -cfeat : implementation of the four hands series }
  ...
  begin
    lStream := TSimpleCommon.OleVariantToMemoryStream(aValue);
    try
      lStream.Position := 0;
      TImage(aComponent).Picture.LoadFromStream(lStream);
    finally
      lStream.Clear;
      FreeAndNil(lStream);
    end;
...
begin
  case aProperty.PropertyType.TypeKind of
    ...
    tkClass:
    begin
      if aProperty.GetValue(Pointer(aEntity)).AsObject is TStream then
...
{$ENDIF}
{ TODO -oAlessandro -cfeat : implementation of the four hands series }
  if ((aComponent is TImage) and (TImage(aComponent).Picture <> nil)) then
    TImage(aComponent).Picture.SaveToStream(TMemoryStream(Result.asObject));
    ...
    begin
      ...
      begin
        __BindValueToComponent(aForm.FindComponent(prpRtti.Name),
          __GetRTTIPropertyValue(aEntity, prpRtti.GetAttribute<Bind>.Field));
      end;
    end;
  finally
    ctxRtti.Free;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          { TODO -oAlessandro -cfeat : implementation of the four hands series }
          for  lAttr in prpRtti.GetAttributes do
          begin
            ...
            begin
              case prpRtti.PropertyType.TypeKind of
                ...
                tkClass:
                begin
                  if (prpRtti.GetValue(Pointer(aEntity)).AsObject is TStream) then
                  begin
                    lStream := TMemoryStream.Create;
                    TBlobField(Field).SaveToStream(lStream);
                    Value := lStream;
                  end;
                end;
                tkMethod:;
      ...
      begin
        ...
        begin
          ...
          begin
            if (lAttr is Campo) then
            begin
              if Uppercase(Campo(lAttr).Name) = UpperCase(Field.DisplayName) then
              begin
                case prpRtti.PropertyType.TypeKind of
                  tkUnknown, tkString, tkWChar, tkLString, tkWString,
                  tkUString: Value := Field.AsString;
                  ...
                  tkClass:
                  begin
                    if (prpRtti.GetValue(Pointer(aList[Pred(aList.Count)])).AsObject
                          is TStream) then
                    begin
                      lStream := TMemoryStream.Create;
                      TBlobField(Field).SaveToStream(lStream);
  ...
  try
    ...
    begin
      ...
      case prpRtti.PropertyType.TypeKind of
            ...
            begin
              if prpRtti.GetValue(Pointer(FInstance)).AsInteger = 0 then
                ...
                aDictionary.Add(prpRtti.FieldName,
                  prpRtti.GetValue(Pointer(FInstance)).AsInteger);
            end
            else
              aDictionary.Add(prpRtti.FieldName,
                prpRtti.GetValue(Pointer(FInstance)).AsInteger);
        ...
        tkFloat:
          begin
            if prpRtti.GetValue(Pointer(FInstance))
              .TypeInfo = TypeInfo(TDateTime) then
            begin
              if YearOf(StrToDateTime(prpRtti.GetValue(Pointer(FInstance))
                .ToString)) < 1900 then
                aDictionary.Add(prpRtti.FieldName, 'null')
              else
                aDictionary.Add(prpRtti.FieldName,
            ...
            else if prpRtti.GetValue(Pointer(FInstance))
              .TypeInfo = TypeInfo(TDate) then
            begin
              if YearOf(StrToDate(prpRtti.GetValue(Pointer(FInstance)).ToString)
                ) < 1900 then
                aDictionary.Add(prpRtti.FieldName, 'null')
              else
                aDictionary.Add(prpRtti.FieldName,
                  StrToDate(prpRtti.GetValue(Pointer(FInstance)).ToString));
            end
            else if prpRtti.GetValue(Pointer(FInstance))
              .TypeInfo = TypeInfo(TTime) then
              aDictionary.Add(prpRtti.FieldName,
                StrToTime(prpRtti.GetValue(Pointer(FInstance)).ToString))
            else
              aDictionary.Add(prpRtti.FieldName,
                __FloatFormat(prpRtti.GetValue(Pointer(FInstance)).ToString));
            if prpRtti.GetValue(Pointer(FInstance)).TypeInfo = TypeInfo(TStream)
            then
              aDictionary.Add(prpRtti.FieldName,
                (prpRtti.GetValue(Pointer(FInstance)).ToString))
          end;
        ...
        begin
          ...
          if (prpRtti.GetValue(Pointer(FInstance)).AsObject is TStream) then
            aDictionary.Add(prpRtti.FieldName,
            TSimpleCommon.MemoryStreamToOleVariant(
            TMemoryStream(prpRtti.GetValue(Pointer(FInstance)).AsObject)));
...
function TSimpleRTTI<T>.GetDataType(const aCampo: String; var aType: TFieldType)
  : iSimpleRTTI<T>;
var
...
begin
  ...
  try
    ...
    begin
      for lAttr in ltypRtti.GetAttributes do
      begin
        if lAttr is Campo then
        begin
          if UpperCase(Campo(lAttr).Name) = UpperCase(aCampo) then
          begin
            aType := Campo(lAttr).Tipo;
            Break;
          end;
        end;
      end;
...
begin
  ...
  try
    ...
    begin
      ...
      aUpdate := aUpdate + prpRtti.FieldName + ' = :' +
        prpRtti.FieldName + ', ';
    end;
  finally
...
begin
  ...
  try
    ...
    begin
      if prpRtti.EhChavePrimaria then
        aWhere := aWhere + prpRtti.FieldName + ' = :' + prpRtti.FieldName
          + ' AND ';
    end;
  finally
