...
{$ENDIF}
    ...
    function FillParameterWhere(aInstance: T; var aWhere: String)
      : iSimpleDAO<T>;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        { TODO -oAlessandro -cfeat : implementation of the four hands series }
        TSimpleRTTI<T>.New(aInstance).GetDataType(Key, lTipo);
        case lTipo of
          ftUnknown,
          ftString:   FQuery.Params.ParamByName(Key).AsString := DictionaryFields.Items[key];
          ftSmallint: FQuery.Params.ParamByName(Key).AsSmallInt := DictionaryFields.Items[key];
          ftInteger:  FQuery.Params.ParamByName(Key).AsInteger := DictionaryFields.Items[key];
          ftWord:     FQuery.Params.ParamByName(Key).AsWord := DictionaryFields.Items[key];
          ftBoolean:  FQuery.Params.ParamByName(Key).AsBoolean := DictionaryFields.Items[key];
          ftFMTBcd,
          ftFloat:    FQuery.Params.ParamByName(Key).AsFloat := DictionaryFields.Items[key];
          ftCurrency: FQuery.Params.ParamByName(Key).AsCurrency := DictionaryFields.Items[key];
          ftBCD:      FQuery.Params.ParamByName(Key).AsBCD := DictionaryFields.Items[key];
          ftDate:     FQuery.Params.ParamByName(Key).AsDate := DictionaryFields.Items[key];
          ftTime:     FQuery.Params.ParamByName(Key).AsTime := DictionaryFields.Items[key];
          ftTimeStamp,
          ftDateTime: FQuery.Params.ParamByName(Key).AsDateTime := DictionaryFields.Items[key];
          ftBytes:    FQuery.Params.ParamByName(Key).AsBytes := DictionaryFields.Items[key];
          ftBlob:
          begin
            lStream := TSimpleCommon.OleVariantToMemoryStream(DictionaryFields.Items[key]);
            try
              FQuery.Params.ParamByName(Key).LoadFromStream(lStream, ftBlob);
            finally
              lStream.Clear;
              FreeAndNil(lStream);
            end;
          ...
          ftWideMemo,
          ftMemo,
          ftWideString: FQuery.Params.ParamByName(Key).AsWideString := DictionaryFields.Items[key];
          ftLargeint:   FQuery.Params.ParamByName(Key).AsLargeInt := DictionaryFields.Items[key];
          ftLongWord:   FQuery.Params.ParamByName(Key).AsLongWord := DictionaryFields.Items[key];
          ftShortint:   FQuery.Params.ParamByName(Key).AsShortInt := DictionaryFields.Items[key];
          ftByte:       FQuery.Params.ParamByName(Key).AsByte := DictionaryFields.Items[key];
          else
            raise Exception.Create('Parameter type not implemented ');
        end;
      end;
...
function TSimpleDAO<T>.FillParameterWhere(aInstance: T; var aWhere: String)
  : iSimpleDAO<T>;
var
  Key, PK: String;
  DictionaryFields: TDictionary<String, Variant>;
